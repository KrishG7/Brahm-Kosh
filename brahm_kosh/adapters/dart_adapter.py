"""
Dart Adapter for Brahm-Kosh.

Parses .dart files using pure-Python regex analysis.
Extracts:
...
SKIP_DIRS = {
    ".git", ".dart_tool", "build", "ios", "android", "web", "linux", "macos", "windows"
}
...
_RE_METHOD_DECL = re.compile(
    r"^[ \t]*(?:(?:static|factory|get|set|late|final|const)\s+)*"
    r"(?:[\w<>,_\[\]?]+\s+)+" # Return type
    r"(_?\w+)\s*\([^)]*\)"
    r"(?:\s*async\*?|\s*sync\*)?"
    r"\s*[\{=]",
...
_KEYWORDS = {
    "if", "for", "while", "switch", "return", "catch", "try", 
    "class", "mixin", "extension", "new", "throw", "void", "super", "this", "case",
    "await", "yield", "var", "final", "const", "late", "get", "set"
}


def _find_block_end(lines: list[str], start: int) -> int:
    # Handle => arrow functions which don't use {}
    if "=>" in lines[start]:
        return start
...
def analyze_directory(root_path: str) -> Project:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        lang_files = sorted([
            ...
            for f in filenames
            if Path(f).suffix.lower() in DART_EXTENSIONS
        ])
        if lang_files:
    ...
    project.metadata.languages = ["Dart"]
    return project

# Self-register
from brahm_kosh.adapters.registry import register_adapter
register_adapter("dart", analyze_directory, extensions=["dart"])
