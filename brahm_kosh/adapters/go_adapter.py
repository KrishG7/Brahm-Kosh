"""
Go Adapter for Brahm-Kosh.

Parses .go files using pure-Python regex analysis.
Extracts:
  - Struct declarations
  - Func declarations (including methods tied to structs)
"""
...
_KEYWORDS = {
    "if", "for", "switch", "select", "return", "defer", "go",
    "type", "func", "struct", "interface", "case", "make", "new",
    "panic", "recover", "append", "len", "cap"
}
...
def _parse_symbols(source: str, lines: list[str]) -> list[Symbol]:
    ...
    for m in _RE_STRUCT_DECL.finditer(source):
        ...
        symbols.append(Symbol(
            name=struct_name,
            kind=SymbolKind.CLASS, # Standardizing struct as CLASS internally
    ...
    # Parse Functions (Go methods are technically package-level funcs with a receiver)
    for m in _RE_FUNC_DECL.finditer(source):
        ...
        start_line = source[:m.start()].count("\n")
        # Ensure it has a `{` to indicate a body (skip declarations)
        end_line = _find_block_end(lines, start_line)
        if end_line == len(lines)-1 and "{" not in lines[start_line]:
            continue # Likely an interface func signature
...
def analyze_directory(root_path: str) -> Project:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        lang_files = sorted([
            ...
            for f in filenames
            if Path(f).suffix.lower() in GO_EXTENSIONS
        ])
        if lang_files:
    ...
    project.metadata.languages = ["Go"]
    return project

# Self-register
from brahm_kosh.adapters.registry import register_adapter
register_adapter("go", analyze_directory, extensions=["go"])
