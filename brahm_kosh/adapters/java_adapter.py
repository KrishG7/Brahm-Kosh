"""
Java Adapter for Brahm-Kosh.

Parses .java files using pure-Python regex analysis.
Extracts:
...
SKIP_DIRS = {
    ".git", "build", "dist", "target", "out", ".idea", ".vscode",
}
...
_RE_METHOD_DECL = re.compile(
    r"^[ \t]+(?:(?:public|protected|private|abstract|static|final|synchronized|native|strictfp)\s+)*"
    r"(?:[\w<>,\[\]?]+\s+)+" # Return type
    r"(\w+)\s*\([^)]*\)"
    r"(?:\s*throws\s+[\w,\s]+)?"
    r"\s*\{",
...
_KEYWORDS = {
    "if", "for", "while", "switch", "return", "catch", "try", "synchronized",
    "class", "interface", "new", "throw", "void", "super", "this", "case",
}
...
def analyze_directory(root_path: str) -> Project:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        lang_files = sorted([
            ...
            for f in filenames
            if Path(f).suffix.lower() in JAVA_EXTENSIONS
        ])
        if lang_files:
    ...
    project.metadata.languages = ["Java"]
    return project

# Self-register
from brahm_kosh.adapters.registry import register_adapter
register_adapter("java", analyze_directory, extensions=["java"])
