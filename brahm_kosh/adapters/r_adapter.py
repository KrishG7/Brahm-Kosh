"""
R Adapter for Brahm-Kosh.

Parses .R files using pure-Python regex analysis.
Extracts:
  - Function declarations (e.g. `name <- function(...)`)
"""
...
SKIP_DIRS = {
    ".git", ".Rproj.user", "renv", "packrat", "data", "inst"
}
...
_KEYWORDS = {
    ...
    "function", "return", "switch", "library", "require",
    "in", "TRUE", "FALSE", "NULL", "NA", "NaN", "Inf"
}


def _find_block_end(lines: list[str], start: int) -> int:
    # R functions might not even use {} if they are one-liners
    # e.g., f <- function(x) x * 2
...
def analyze_directory(root_path: str) -> Project:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        ...
        lang_files = sorted([
            ...
            for f in filenames
            if Path(f).suffix.lower() in R_EXTENSIONS
        ])
        if lang_files:
    ...
    project.metadata.languages = ["R"]
    return project

# Self-register
from brahm_kosh.adapters.registry import register_adapter
register_adapter("r", analyze_directory, extensions=["r"])
